...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/figures/lm7-compute-dark.svg">
  <img alt="LM7 layers for CPU and GPU: model, orchestrator, backends per target, lowering and runtime, hardware" src="docs/figures/lm7-compute-light.svg">
</picture>

Every CPU reaches LM7 through the same `cpu` target, Intel, AMD, Apple and Arm
alike. A vendor column shows its CPU beside its accelerators because that is the
machine you actually have, not because the target string differs.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/figures/lm7-accelerators-dark.svg">
  <img alt="LM7 layers for Google TPU, Tenstorrent, and Qualcomm Snapdragon" src="docs/figures/lm7-accelerators-light.svg">
</picture>

Both figures are generated from `docs/figures/generate_architecture.py`, so the
targets and backends in them are kept in one place rather than redrawn by hand.
