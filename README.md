...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/figures/lm7-architecture-dark.svg">
  <img alt="LM7 in five layers: the PyTorch model, the LM7 orchestrator, backends per target, lowering and runtime, and the hardware — one column per vendor" src="docs/figures/lm7-architecture-light.svg">
</picture>
...
The figure is generated by `docs/figures/generate_architecture.py`, so a target
or backend is corrected in one table rather than redrawn.
