...
def _same_device(tensor_device: torch.device, target_device: torch.device) -> bool:
    ...
    ``torch.device("mps") != torch.device("mps", 0)``, and moving a tensor to the
    former produces the latter -- so a plain ``!=`` made ``transfers="explicit"``
    impossible to satisfy on every indexed device. The caller could not win:
    placing the input exactly where LM7 asked still failed the check. An
    unindexed device means "the current one", which is index 0 here.
    """
