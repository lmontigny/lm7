...
| `onnxruntime` | CPU and NVIDIA CUDA. Tensor-only inputs and flat outputs, bound through ORT's I/O binding, so a CUDA session takes inputs from either device and leaves its outputs on the one that produced them. Weights above protobuf's 2 GiB message limit move to a `compiled_model.onnx.data` sidecar carried as a second checksummed payload. **Dynamic causal-LM sequence lengths and KV-cache decode graphs remain unclaimed**, so this is an export-and-forward backend, not a generation one. See the [guide](onnxruntime.md). |
