...
| `cpu:x86_64` (Intel) | Intel Xeon Platinum 8581C (Emerald Rapids), GCP `c4-standard-8`, 8 vCPU / 4 physical cores, AVX-512 BF16/VNNI and AMX BF16/INT8 | Detection, backend planning, and a tiny `lm7.compile(..., target="cpu", backend="auto", fallback="error")` TorchInductor correctness smoke test. No benchmark run. |
