...
def render(theme: Theme, vendors: tuple[str, ...]) -> str:
    ...
    s.append(_logo("pytorch", x0 + 20, y_model + 18, 32, theme))
    s.append(_t(x0 + 54, y_model + 26, "PyTorch / Hugging Face model", 18, theme.ink, "700"))
    s.append(
        _t(
            x0 + 54,
            y_model + 44,
    ...
    s.append(
        _t(
            x0 + grid_w / 2,
            height - 10,
            "eager is the fallback everywhere and is omitted here; backend priority "
            "and artifact formats are in the table below",
            8.5,
            theme.muted,
            anchor="middle",
        )
