...
the figure is the new runs.
    ...
    for run in 1 2 3 4 5 6 7; do
      python benchmarks/gpu.py --target nvidia --model smollm2 --dtype float16 \\
        --backend torch-compile inductor-placed inductor --repeats 100 \\
        --output artifacts/overhead-smollm2-cuda-run$run.json
    done
    python docs/figures/overhead.py

writes `lm7-overhead-rtx.svg` and, when cairosvg is available, a 2x
`lm7-overhead-rtx.png` beside this script. The README uses the PNG, which renders
...
BAR_H = 26
X_MAX = 30.0  # milliseconds; the axis starts at zero, because a bar chart must
...
def main() -> None:
    ...
    parser.add_argument("--glob", default="overhead-smollm2-cuda-run*.json")
